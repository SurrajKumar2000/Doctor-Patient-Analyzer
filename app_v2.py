...
def transcribe_with_diarization(audio_path):
    ...
    with open(audio_path,"rb") as f:
        ...
        data={"model":"whisper-large-v3","response_format":"verbose_json","language":"en","temperature":"0"}
    ...
    result=resp.json(); segments=result.get("segments",[]); full_text=result.get("text","")
    utterances=[]; current_speaker="A"
    for i,seg in enumerate(segments):
        text=seg.get("text","").strip()
        start_ms=int(seg.get("start",0)*1000); end_ms=int(seg.get("end",0)*1000)
        if i>0:
            prev=segments[i-1]; prev_text=prev.get("text","").strip(); gap=seg.get("start",0)-prev.get("end",0)
            if prev_text.endswith("?") or gap>0.8:
                current_speaker="B" if current_speaker=="A" else "A"
        utterances.append({"speaker":current_speaker,"start":start_ms,"end":end_ms,"text":text,"confidence":seg.get("avg_logprob",0)})
...
def identify_speakers(utterances):
    ...
    doctor_label=max(qc,key=qc.get) if qc else "A"
    all_speakers=sorted({u["speaker"] for u in utterances})
    patient_label=next((s for s in all_speakers if s!=doctor_label),"B")
...
def process_diarized_transcript(raw_transcript):
    ...
    for utt in raw_transcript.get("utterances",[]):
        processed.append({"speaker":utt.get("speaker","?"),
...
def render_feedback_block(title, icon, data):
    """Scores are always pre-pinned to TOPSIS before calling this."""
...
def pinned_block(feedback, fb_key, topsis_key, topsis):
    """Return feedback block with score always pinned to TOPSIS value."""
...
with st.sidebar:
    ...
    groq_key=st.text_input("Groq API Key",type="password",help="Free at console.groq.com",placeholder="gsk_...")
    if groq_key: st.session_state["groq_key_store"]=groq_key
    if groq_key and not groq_key.strip().startswith("gsk_"): st.warning("⚠️ Groq keys start with **gsk_**")
    st.divider()
    ...
    <div style="background:#f0fff4;border:1px solid #9ae6b4;border-radius:8px;
        ...
        • Speaker-separated transcription<br>
    ...
    - Scores pinned to algorithm (not AI guesses)
    - Doctor-only coaching
    - Cross-modal behavioural insights

    ### 🔑 Free Key
    - **console.groq.com** (Groq only)
    """)
    st.divider()
    st.caption("v2 — Multi-Modal Framework PoC")
...
<div class="main-header">
    ...
    <p>v2 · MediaPipe + Groq Whisper + Groq Llama 3.3 70B · TOPSIS Calgary-Cambridge Scoring</p>
</div>""", unsafe_allow_html=True)
...
if analyze_btn and youtube_url:
    ...
    utterances=[]; speaker_map={"doctor":"A","patient":"B"}; speaker_stats={}; audio_path=None
    with st.status("🎙️ Transcribing + splitting speakers…",expanded=True) as audio_status:
        try:
            ...
            doc=speaker_stats["doctor"]; pat=speaker_stats["patient"]
            audio_status.update(label=f"✅ Doctor: {doc['word_count']} words, {doc['wpm']} WPM, {doc['filler_count']} fillers | Patient: {pat['word_count']} words, {pat['wpm']} WPM | Avg latency: {speaker_stats['avg_response_latency']}s",state="complete")
        except Exception as e:
            audio_status.update(label=f"⚠️ Diarization failed: {e}",state="error")
            speaker_stats={"doctor":{"word_count":0,"wpm":0,"filler_count":None,"turn_count":0,"avg_turn_duration":0,"label":"A"},
                           "patient":{"word_count":0,"wpm":0,"filler_count":0,"turn_count":0,"avg_turn_duration":0,"label":"B"},
                           "response_latencies":[],"avg_response_latency":None,"total_turns":0,"doctor_turn_pct":None,"patient_turn_pct":None,"_diarization_failed":True}
    ...
    feedback=None
    with st.spinner("🤖 Generating AI feedback via Groq…"):
    ...
    # ── Always force score to TOPSIS value — LLM cannot change this ──
    if feedback:
    ...
    # ══════════════════════════════════════════
        ...
        if utterances:
            ...
            st.caption(f"Showing first 6 turns · 🔵 Doctor · 🟢 Patient")
        ...
        # ── DETAILED FEEDBACK (scores pinned to TOPSIS) ──
        ...
        with st.expander("🗃️ Raw Analysis Data (JSON)"):
            st.json({"algorithm":"TOPSIS + Calgary-Cambridge",
                     "topsis_result":{k:v for k,v in topsis.items() if k!="breakdown"},
                     "topsis_breakdown":{k:{"label":v["label"],"raw_value":v["raw_value"],"normalized":v["normalized"],"weighted":v["weighted"],"weight":v["weight_pct"],"dimension":v["dimension"]} for k,v in topsis["breakdown"].items()},
                     ...
                     "utterances":utterances[:20],"ai_feedback":feedback})
...
st.markdown('<div style="text-align:center;color:#a0aec0;font-size:.8rem;margin-top:3rem;padding:1rem;border-top:1px solid #e2e8f0">v2 · Multi-Modal Communication Framework PoC · MediaPipe + Groq Whisper + Groq (Llama 3.3 70B) · TOPSIS + Calgary-Cambridge Scoring</div>', unsafe_allow_html=True)
