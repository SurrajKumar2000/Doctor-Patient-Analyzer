...
# ─────────────────────────────────────────────
# AUDIO EXTRACTION (video → WAV for PyAnnote)
# ─────────────────────────────────────────────
def extract_audio(video_path, output_dir):
    """
    Extract audio as WAV (16kHz mono).
    WAV is required by PyAnnote — it cannot read MP3 reliably.
    Groq Whisper also accepts WAV without issues.
    """
    wav_path = os.path.join(output_dir, "audio.wav")
    result = subprocess.run(
        ["ffmpeg", "-i", video_path, "-vn", "-ar", "16000", "-ac", "1", "-y", wav_path],
...
# ─────────────────────────────────────────────
# GROQ WHISPER + PYANNOTE DIARIZATION
# ─────────────────────────────────────────────

def _merge_whisper_pyannote(whisper_segments, pyannote_segments):
    """
    Bridge between Whisper and PyAnnote using timestamps.
    For each Whisper text segment, find the PyAnnote speaker
    with maximum timestamp overlap and assign that label.
    """
    utterances = []
...
def transcribe_with_diarization(audio_path):
    """
    Groq Whisper + PyAnnote diarization pipeline.

    Step 1 → Groq Whisper (cloud): text + timestamps
    Step 2 → PyAnnote 3.1 (local): speaker labels + timestamps
    Step 3 → Merge via timestamp overlap
...
# ─────────────────────────────────────────────
# Replaces the old question-mark counting heuristic.
# Sends transcript to LLM which reads the CONTENT
# to identify who is the doctor and who is the patient.
# ─────────────────────────────────────────────
...
def identify_speakers(utterances):
    ...
    Strategy:
    1. Try LLM — reads content like a human (most accurate)
    2. Fallback — first speaker is usually the doctor
    """
    if not utterances:
        return {"doctor": "A", "patient": "B"}

    all_speakers = sorted({u["speaker"] for u in utterances})
    if len(all_speakers) < 2:
        return {"doctor": all_speakers[0], "patient": "SPEAKER_01"}

    # Fallback: first speaker = doctor (used if LLM fails)
    ...
    # Build a short transcript sample for LLM to read
    sample = utterances[:20]
    transcript_lines = "\n".join(
        f"{u['speaker']} ({u['start_s']}s): {u['text'][:100]}"
        for u in sample
    )
...
doctor={all_speakers[1]}"""
    ...
    try:
        ...
        # If LLM response doesn't match expected format, use fallback
        return fallback
...
def analyze_per_speaker(utterances, speaker_map):
    ...
    def stats(utts):
        if not utts:
            return {"word_count": 0, "wpm": 0, "filler_count": 0, "turn_count": 0, "avg_turn_duration": 0}
...
# ─────────────────────────────────────────────
# GROQ LLM — LLAMA 3.3 70B (FEEDBACK)
# ─────────────────────────────────────────────
...
with st.sidebar:
    ...
    <div style="background:#f0fff4;border:1px solid #9ae6b4;border-radius:8px;
        ...
        • LLM → speaker identification (reads content)<br>
        • Audio: WAV 16kHz mono<br>
    ...
    ### 🤖 AI Feedback
    - LLM identifies doctor vs patient
    ...
    """)
    st.divider()
    st.caption("v3 — Groq Whisper + PyAnnote + LLM ID + TOPSIS")
...
<div class="main-header">
    ...
    <p>v3 · Groq Whisper + PyAnnote (num_speakers=2) + LLM Speaker ID + Groq Llama 3.3 70B · TOPSIS Calgary-Cambridge</p>
</div>""", unsafe_allow_html=True)
...
if analyze_btn and youtube_url:
    ...
    with st.status("🎙️ Groq Whisper + PyAnnote diarization…", expanded=True) as audio_status:
        try:
            ...
            # STEP 3.5 — LLM identifies doctor vs patient
            ...
            method_label = (
                f"PyAnnote (~94%) · {raw.get('device_used', '')}"
                if diarization_method == "pyannote"
                else "Heuristic (~60%) — add HF token for better results"
            )
            audio_status.update(
                label=(
                    f"✅ Doctor ({speaker_map['doctor']}): {doc['word_count']}w, {doc['wpm']} WPM, "
                    f"{doc['filler_count']} fillers | "
                    f"Patient ({speaker_map['patient']}): {pat['word_count']}w, {pat['wpm']} WPM | "
                    f"Latency: {speaker_stats['avg_response_latency']}s | "
                    f"{method_label}"
                ),
                state="complete"
            )
    ...
    # STEP 4 — TOPSIS scoring (calculated ONCE)
    ...
    # Switching Quick ↔ Research reads from here
    # Same data, same scores — always
    # ─────────────────────────────────────────
...
# Both modes read the EXACT SAME stored data
# Score is identical regardless of mode
# ─────────────────────────────────────────────
if "results" in st.session_state:
    ...
    st.success(f"✅ Results for **{title}**")
    st.divider()
    ...
    # ══════════════════════════════════════════
        ...
        if utterances:
            ...
            st.caption(f"Showing first 6 turns · 🔵 Doctor ({speaker_map['doctor']}) · 🟢 Patient ({speaker_map['patient']}) · Diarization: {diarization_method.upper()} · Speaker ID: LLM")
        ...
        with st.expander("🗃️ Raw Analysis Data (JSON)"):
            st.json({
                ...
                "diarization_method" : diarization_method,
                "speaker_id_method"  : "LLM (Llama 3.3 70B)",
                "speaker_map"        : speaker_map,
...
st.markdown('<div style="text-align:center;color:#a0aec0;font-size:.8rem;margin-top:3rem;padding:1rem;border-top:1px solid #e2e8f0">v3 · Groq Whisper + PyAnnote (num_speakers=2) + LLM Speaker ID + Groq Llama 3.3 70B · TOPSIS + Calgary-Cambridge</div>', unsafe_allow_html=True)
