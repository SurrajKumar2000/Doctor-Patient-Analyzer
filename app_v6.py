...
# ─────────────────────────────────────────────
st.set_page_config(
    page_title="Doctor-Patient Communication Analyzer v5",
    page_icon="🩺",
    layout="wide",
...
.vikor-box {
    background: linear-gradient(135deg, #fff5f7, #ebf8ff);
    border: 1px solid #f687b3; border-radius: 12px;
...
# ─────────────────────────────────────────────
# CALGARY-CAMBRIDGE CRITERIA
# Same weights as v4 — only scoring algorithm changes (VIKOR replaces TOPSIS)
# ─────────────────────────────────────────────
CRITERIA = {
...
# ─────────────────────────────────────────────
# VIKOR SCORING — replaces TOPSIS in v5
#
# VIKOR = VIseKriterijumska Optimizacija I Kompromisno Resenje
# (Multi-Criteria Optimization and Compromise Solution)
#
# Key difference from TOPSIS:
#   TOPSIS  = distance from ideal/worst (geometric)
#   VIKOR   = maximum group utility + minimum individual regret
#
# Formula:
#   S  = Σ w_j * (f*_j - f_j) / (f*_j - f-_j)   [utility measure]
#   R  = max[ w_j * (f*_j - f_j) / (f*_j - f-_j) ] [regret measure]
#   Q  = v*(S-S*)/(S- -S*) + (1-v)*(R-R*)/(R- -R*)  [compromise score]
#
# v = 0.5 (equal weight to utility and regret — standard compromise)
# Final score = (1 - Q) * 100  [inverted so higher = better]
# ─────────────────────────────────────────────
def calculate_vikor_score(visual_stats, speaker_stats, v_weight=0.5):
    """
    VIKOR multi-criteria scoring using Calgary-Cambridge framework.

    Args:
        visual_stats:  MediaPipe output dict
        speaker_stats: Diarized speaker analysis dict
        v_weight:      Strategy weight (0=regret focused, 1=utility focused, 0.5=compromise)

    Returns:
        dict with vikor_score, grade, S, R, Q, breakdown per dimension
    """
    # ── Step 1: Raw values (same as v4) ──────────────────────────
    raw = {
    ...
    # ── Step 2: Ideal (f*) and Worst (f-) per criterion ──────────
    f_star = {k: CRITERIA[k]["ideal"] for k in CRITERIA}   # ideal best
    f_minus = {k: CRITERIA[k]["worst"] for k in CRITERIA}  # ideal worst

    # ── Step 3: Normalized weighted distances ────────────────────
    # d_j = w_j * |f*_j - f_j| / |f*_j - f-_j|
    d = {}
    for key in CRITERIA:
        rang = abs(f_star[key] - f_minus[key])
        if rang == 0:
            d[key] = 0.0
        else:
            d[key] = CRITERIA[key]["weight"] * abs(f_star[key] - raw[key]) / rang

    # ── Step 4: S (utility) and R (regret) ───────────────────────
    S = sum(d.values())           # group utility — sum of all weighted distances
    R = max(d.values())           # maximum individual regret — worst single criterion

    # ── Step 5: Reference points ─────────────────────────────────
    # Best possible: S*=0, R*=0 (all criteria at ideal)
    # Worst possible: S-=sum(weights)=1.0, R-=max(weights)=0.25
    S_star = 0.0
    S_minus = sum(CRITERIA[k]["weight"] for k in CRITERIA)   # = 1.0
    R_star = 0.0
    R_minus = max(CRITERIA[k]["weight"] for k in CRITERIA)   # = 0.25

    # ── Step 6: Q (compromise score) ─────────────────────────────
    # Q = v*(S-S*)/(S--S*) + (1-v)*(R-R*)/(R--R*)
    s_term = (S - S_star) / (S_minus - S_star) if (S_minus - S_star) > 0 else 0
    r_term = (R - R_star) / (R_minus - R_star) if (R_minus - R_star) > 0 else 0
    Q = v_weight * s_term + (1 - v_weight) * r_term
    Q = round(min(max(Q, 0.0), 1.0), 4)

    # ── Step 7: Final score (inverted — higher Q = worse) ────────
    # Convert to 0-100 where 100 = perfect
    final_score = round((1 - Q) * 100, 1)
    grade = ("Excellent" if final_score >= 80 else "Good" if final_score >= 65
             else "Fair" if final_score >= 45 else "Needs Improvement")

    # ── Step 8: Per-dimension normalized scores (for display) ─────
    # Convert each dimension's contribution to 0-100 scale
    normalized = {}
    for key in CRITERIA:
        rang = abs(f_star[key] - f_minus[key])
        if rang == 0:
            normalized[key] = 100.0
        else:
            # How close to ideal? 0=worst, 100=ideal
            normalized[key] = round(max(0, min(100, (1 - abs(f_star[key] - raw[key]) / rang) * 100)), 1)

    # ── Step 9: Breakdown for UI display ─────────────────────────
    breakdown = {}
    for key in normalized:
        ...
        breakdown[key] = {
            ...
            "d_j"        : round(d[key], 4),       # weighted distance contribution
            "weight_pct" : f"{int(CRITERIA[key]['weight']*100)}%",
            "dimension"  : CRITERIA[key]["dimension"],
    ...
    return {
        ...
        "raw"         : raw,
        "d"           : {k: round(v, 4) for k, v in d.items()},
    }

def radar_chart(vikor_result):
    labels = [CRITERIA[k]["label"] for k in CRITERIA]
    values = [vikor_result["normalized"][k] for k in CRITERIA]
    vc = values + [values[0]]; lc = labels + [labels[0]]
    fig = go.Figure()
    fig.add_trace(go.Scatterpolar(r=vc, theta=lc, fill="toself",
        fillcolor="rgba(214,93,177,0.15)", line=dict(color="#b83280",width=2), name="Doctor Score"))
...
# ─────────────────────────────────────────────
# V4 METRICS — unchanged from v4
# ─────────────────────────────────────────────
def calculate_patient_engagement(utterances, speaker_map):
    """Patient-initiated elaboration rate."""
    if not utterances:
...
def calculate_hesitation_windows(speaker_stats, threshold=2.0):
    """Patient pauses flagged with clinical context."""
    windows = []
...
def calculate_brow_furrow_index(visual_stats):
    """Brow furrow = patient confusion signal."""
...
def calculate_session_arc(utterances, speaker_map):
    """First half vs second half quality."""
    if len(utterances) < 4:
...
# ─────────────────────────────────────────────
# MEDIAPIPE HELPERS — unchanged from v4
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# VIDEO ANALYSIS — unchanged from v4
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# AUDIO EXTRACTION — unchanged from v4
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# SPEAKER IDENTIFICATION — unchanged from v4
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# YOUTUBE DOWNLOAD — unchanged from v4
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# GROQ LLM — updated prompt to reference VIKOR
# ─────────────────────────────────────────────
GROQ_CHAT_URL="https://api.groq.com/openai/v1/chat/completions"
GROQ_MODEL   ="llama-3.3-70b-versatile"

def build_prompt(visual_stats, speaker_stats, utterances, speaker_map,
                 aligned, vikor, engagement, hesitation_windows):
    ...
    bd_str="\n".join(f"  {v['icon']} {v['label']} ({v['weight_pct']}): {v['normalized']}/100"
                     for k,v in vikor["breakdown"].items())
    ...
    aligned_str="\n".join(f"  [{ev['time']}] {ev['role']}: \"{ev['speech'][:60]}\"\n    Eye: {ev['eye_contact']} | Expr: {ev['expression']}"
                          for ev in aligned[:8])
    locked_score = vikor["vikor_score"]
    return f"""You are an expert clinical communication coach analysing a doctor-patient interaction.
Score is calculated via VIKOR + Calgary-Cambridge — you MUST use this exact value: {locked_score}/100.

━━━ VIKOR SCORE (LOCKED) ━━━
Overall: {locked_score}/100 — {vikor['grade']}
S (Group Utility): {vikor['S']} | R (Max Regret): {vikor['R']} | Q (Compromise): {vikor['Q']}
v-weight: {vikor['v_weight']} (balanced utility-regret compromise)
{bd_str}
...
Note: R={vikor['R']} is the maximum regret criterion — the single worst performing dimension.
empathy_score: rate 0-100 how empathetic, warm, patient-centred the doctor was.
Return ONLY valid JSON, no markdown fences:

{{
  "overall_score": {locked_score},
  "vikor_explanation": "<2 sentences explaining S, R, Q values and what they mean clinically>",
  "regret_insight": "<1 sentence identifying the highest regret criterion and its clinical impact>",
  ...
  "eye_contact_feedback": {{"score":{round(vikor['breakdown']['eye_contact']['normalized'])},"assessment":"<impact>","recommendations":["<tip>","<tip>","<tip>"]}},
  "doctor_speech_feedback": {{"score":{round(vikor['breakdown']['speech_clarity']['normalized'])},"assessment":"<impact>","recommendations":["<tip>","<tip>","<tip>"]}},
  "listening_feedback": {{"score":{round(vikor['breakdown']['turn_balance']['normalized'])},"assessment":"<impact>","recommendations":["<tip>","<tip>"]}},
  "body_language_feedback": {{"score":{round(vikor['breakdown']['body_language']['normalized'])},"assessment":"<impact>","recommendations":["<tip>","<tip>"]}},
  "patient_impact_analysis": {{"score":{round(vikor['breakdown']['response_latency']['normalized'])},"assessment":"<impact>","recommendations":["<tip>","<tip>"]}},
...
def get_llm_feedback(visual_stats, speaker_stats, utterances, speaker_map,
                     aligned, vikor, engagement, hesitation_windows, groq_key):
...
def pinned_block(feedback, fb_key, vikor_key, vikor):
    block=dict(feedback.get(fb_key,{}))
    block["score"]=round(vikor["breakdown"][vikor_key]["normalized"])
    return block
...
with st.sidebar:
    ...
    st.divider()
    VIKOR_V_WEIGHT = 0.5  # Fixed compromise — equal utility and regret
    ...
    st.divider()
    st.markdown("""
    <div style="background:#fff5f7;border:1px solid #f687b3;border-radius:8px;
                padding:.8rem 1rem;font-size:.82rem;color:#702459">
        ⚙️ <b>v5 Pipeline:</b><br>
        • Groq Whisper → transcription (MP3)<br>
        • PyAnnote → diarization (WAV)<br>
        • <b>VIKOR + Calgary-Cambridge scoring</b><br>
        • S (utility) · R (regret) · Q (compromise)<br>
    ...
    st.divider()
    st.caption("v5 — VIKOR · Multi-Modal Clinical Communication Analyzer")
...
<div class="main-header">
    ...
    <p>v5 · MediaPipe + Groq Whisper + PyAnnote + Groq Llama 3.3 70B · VIKOR Calgary-Cambridge</p>
</div>""",unsafe_allow_html=True)
...
if analyze_btn and youtube_url:
    ...
    # ── VIKOR scoring (replaces TOPSIS) ──────
    with st.spinner("📐 Calculating VIKOR + Calgary-Cambridge…"):
        vikor = calculate_vikor_score(visual_stats, speaker_stats, v_weight=0.5)
    ...
    # ── SCORE LOCK — VIKOR score is immutable ─
    if feedback:
        feedback["overall_score"]=vikor["vikor_score"]
        feedback["overall_grade"]=vikor["grade"]
...
if "analysis_results" in st.session_state:
    ...
    title              = R["title"]
    v_weight           = R.get("v_weight", 0.5)

    OVERALL_SCORE = vikor["vikor_score"]
    OVERALL_GRADE = vikor["grade"]
    ...
    st.markdown(
        ...
        f'VIKOR + Calgary-Cambridge · Overall Score</div>'
        ...
        f'S={vikor["S"]} · R={vikor["R"]} · Q={vikor["Q"]} · v=0.5</div></div>',
        unsafe_allow_html=True)

    st.caption(f"🔒 Score source: VIKOR · S={vikor['S']} R={vikor['R']} Q={vikor['Q']} · Mode: {'Quick' if is_quick else 'Research'}")
    st.divider()
    ...
    if is_quick:
        ...
        for key,val in vikor["breakdown"].items():
        ...
        st.plotly_chart(radar_chart(vikor),use_container_width=True)
        ...
        if feedback and feedback.get("empathy_assessment"):
            st.markdown(f'<div class="engagement-card">💚 <b>Empathy:</b> {feedback["empathy_assessment"]}</div>',
                        unsafe_allow_html=True)
        if feedback and feedback.get("regret_insight"):
            st.markdown(f'<div class="hesitation-card">⚠️ <b>Max Regret Criterion:</b> {feedback["regret_insight"]}</div>',
                        unsafe_allow_html=True)
    ...
    # ══════════════════════════════════════════
        ...
        # ── VIKOR explanation box ─────────────
        st.markdown(
            f'<div class="vikor-box">'
            f'📐 <b>VIKOR Score (Calgary-Cambridge)</b><br><br>'
            f'<b>S (Group Utility) = {vikor["S"]}</b> — sum of weighted distances from ideal (lower = better)<br>'
            f'<b>R (Max Regret) = {vikor["R"]}</b> — worst single criterion gap (lower = better)<br>'
            f'<b>Q (Compromise) = {vikor["Q"]}</b> — final compromise index (lower = better)<br>'
            f'<b>v-weight = {v_weight}</b> — {"balanced" if v_weight==0.5 else "regret-focused" if v_weight<0.5 else "utility-focused"} strategy<br><br>'
            f'{feedback.get("vikor_explanation","") if feedback else ""}</div>',
            unsafe_allow_html=True)

        # Regret insight
        if feedback and feedback.get("regret_insight"):
            st.markdown(
                f'<div class="hesitation-card">⚠️ <b>Maximum Regret Criterion:</b> {feedback["regret_insight"]}</div>',
                unsafe_allow_html=True)
        ...
        with rc:
            ...
            st.plotly_chart(radar_chart(vikor),use_container_width=True)
        with bc:
            st.markdown("#### 📐 VIKOR Dimension Breakdown")
            # Identify max regret criterion for highlighting
            max_regret_key = max(vikor["d"].items(), key=lambda x: x[1])[0]
            for key,val in vikor["breakdown"].items():
                n=val["normalized"]; col="#38a169" if n>=70 else "#d69e2e" if n>=40 else "#e53e3e"
                is_regret = key == max_regret_key
                border = "border:2px solid #e53e3e;" if is_regret else "border:1px solid #e2e8f0;"
                regret_tag = ' <span style="font-size:.7rem;background:#fed7d7;color:#9b2c2c;padding:.1rem .4rem;border-radius:4px;font-weight:700">MAX REGRET</span>' if is_regret else ""
                st.markdown(
                    f'<div style="background:white;border-radius:8px;padding:.7rem 1rem;'
                    f'{border}margin-bottom:.5rem">'
                    ...
                    f'{val["icon"]} {val["label"]} ({val["weight_pct"]}){regret_tag}</span>'
                    ...
                    f'{val["dimension"]} · Raw: {val["raw_value"]} · d_j: {val["d_j"]}</div></div>',
                    unsafe_allow_html=True)
        ...
        if feedback:
            ...
            with fc1:
                render_feedback_block("Eye Contact",     "👁️",pinned_block(feedback,"eye_contact_feedback",   "eye_contact",    vikor))
                render_feedback_block("Doctor's Speech", "🎙️",pinned_block(feedback,"doctor_speech_feedback", "speech_clarity",  vikor))
                render_feedback_block("Listening Skills","👂",pinned_block(feedback,"listening_feedback",      "turn_balance",    vikor))
            with fc2:
                render_feedback_block("Body Language",   "🙆",pinned_block(feedback,"body_language_feedback",  "body_language",   vikor))
                render_feedback_block("Patient Impact",  "💙",pinned_block(feedback,"patient_impact_analysis", "response_latency",vikor))
            ...
            if tips:
                ...
                for i,(tip,col) in enumerate(zip(tips,cols)):
                    with col:
                        st.markdown(
                            f'<div style="background:linear-gradient(135deg,#fff5f7,#e6fffa);'
                            f'border-radius:10px;padding:1rem;text-align:center;'
                            f'border:1px solid #f687b3;height:100%">'
                            f'<div style="font-size:1.5rem">{tip_icons[i%len(tip_icons)]}</div>'
                            f'<div style="font-size:.85rem;color:#702459;margin-top:.5rem;line-height:1.5">{tip}</div></div>',
                            unsafe_allow_html=True)

        with st.expander("🗃️ Raw Analysis Data (JSON)"):
            st.json({
                "version"         : "v5",
                "algorithm"       : "VIKOR + Calgary-Cambridge",
                "score_source"    : "vikor[vikor_score] — immutable",
                "overall_score"   : OVERALL_SCORE,
                "diarization_method": diarization_method,
                "vikor_result"    : {k:v for k,v in vikor.items() if k not in ["breakdown","normalized","raw","d"]},
                "vikor_breakdown" : {k:{"label":v["label"],"raw_value":v["raw_value"],
                                        "normalized":v["normalized"],"d_j":v["d_j"],
...
st.markdown(
    ...
    'v5 · Groq Whisper (MP3) + PyAnnote (WAV · Mac M1 MPS) + Groq Llama 3.3 70B · '
    'VIKOR + Calgary-Cambridge · S/R/Q · Empathy · Engagement · Hesitation · Brow Furrow · Session Arc'
    '</div>',unsafe_allow_html=True)
